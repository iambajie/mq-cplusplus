...
{
    if(ibPush&&m_vSubscribeConsumers.empty())//需要推送并且队列的订阅者为空
    {
        return NULL;
    }
    if(m_msgPriQueue.empty()&&m_messagesList.empty())//存放的优先消息和普通股消息都为空
    {
        return NULL;
    }
    //若优先消息不为空，则推送优先消息
    ...
    //若消息是用来推送，从订阅者中选择一个接收，上次推送给了那一个订阅者，每次都从上次分配的下一个开始
    if(ibPush)
    {
        m_iPushIndex++;
        ...
        pFrontMsg->SetClientIndex(m_vSubscribeConsumers[m_iPushIndex]);//设置客户端下标
    }    
    return pFrontMsg;
}
...
int MessageQueue::AddMessage(SeverStoreMessage * ipMsg)
{
    ...
    if(m_iPriority>0&&ipMsg->GetPriority()>0)
    {
        ...
        if(ipMsg->GetPriority()>m_iPriority)//消息优先级大于队列优先级
        {
            ipMsg->SetPriority(m_iPriority);//设置为队列的优先级
        }
        m_msgPriQueue.push(ipMsg);//将消息放入优先级队列中
    }
    else
    {
        m_messagesList.push_back(ipMsg);//放入普通消息列表中
    }
    return SUCCESS;
}
...
{
    SeverStoreMessage *pFrontMsg=NULL;
    while(!m_msgPriQueue.empty())//清空优先消息
    {
        pFrontMsg=m_msgPriQueue.top();
        m_msgPriQueue.pop();
        delete pFrontMsg;
    }
    while(!m_messagesList.empty()) //清空普通消息
    {
        pFrontMsg=m_messagesList.front();
        m_messagesList.pop_front();
        delete pFrontMsg;
...
int MessageQueue::AddSubscribe(int iCliIndex,unsigned char iConfirmLevel)
{
    ...
    m_iConfirmLevel=iConfirmLevel;//队列中的消息确认级别
    return SUCCESS;
}
...
int MessageQueue::SerializeSubcribeToString(char *ipBuffer,int &iBuffLen,bool ibIsCreate)
{
    ...
    if(!m_vSubscribeConsumers.empty())//队列订阅者不为空
    {
...
{
    //查找是否存在现有绑定
    if(m_mBindingQueues.find(istrBindingKey)!=m_mBindingQueues.end())//根据binding key 绑定的queue
    {
...
bool Exchange::IsFit(const string& istrRoutingKey,const string& iStrBindingKey)
{
    ...
    int lo=0;
    int hi=0;
    //将routingkey根据”.“分割，保存在vStrRoutingKey中
    ...
    lo=0;
    hi=0;
    //将bindingkey根据”.“分割，保存在vStrBindingKey中
...
// ＃（哈希）可以替代零个或多个单词。
    ...
    for(int i=1;i<=iLen1;++i)
    {
        for(int j=1;j<=iLen2;++j)
        {
            ...
            {
                dp[i][j]=dp[i-1][j-1];//等于前一个的结果
            }
            if(vStrRoutingKey[j-1]=="#")
            {
                dp[i][j]=dp[i-1][j]||dp[i][j-1];//等于之前的结果
            }
        }
    }
...
{
     //根据exchange 类型进行处理
    if(m_iExchangeType==EXCHANGE_TYPE_DIRECT)//根据绑定建有选择的接收消息  2
    {
        //直接转发给key匹配队列
    ...
    else if(m_iExchangeType==EXCHANGE_TYPE_FANOUT)//将消息广播  1
    {
        //转发给所有绑定队列
    ...
    else if(m_iExchangeType==EXCHANGE_TYPE_TOPIC)//基于主题接收消息  3
    {
        //匹配查询
        if(m_mBindingQueues.empty())
        {
...
int Exchange::DeleteBindingQueue(MessageQueue *ipMsgQueue)
{
    ...
    while(itr1!=m_mBindingQueues.end())
    {
        ...
        if(queueList.empty())
        {
            m_mBindingQueues.erase(itr1++);//删除当前空的队列对应的绑定键
        }
        else
        {
            ++itr1;//找下一个路由键绑定的队列
...
int LogicServer::InitSigHandler()
{
    ...
    act.sa_handler=SigTermHandler;//新的信号处理函数
    sigaction(SIGINT,&act,NULL);////检查或修改与指定信号相关联的处理动作
...
{
    //查看queue文件夹是否存在
    string strQueueDir=DEFAULT_QUEUE_PATH;//持久化默认路径
    if(!FuncTool::IsFileExist(strQueueDir.c_str()))
    {
        return ERROR;
    }
    //遍历文件夹中的所有子文件夹
    DIR* pDir = opendir(strQueueDir.c_str());  //打开文件目录 
    if(!pDir)
    {
        return ERROR;
    }
...
// 然后，我们遍历d，调用stat（d->name,stat *e）来获取每个文件的详细信息，存储在stat结构体e中。
    struct dirent *dir;//dirent不仅仅指向目录，还指向目录中的具体文件
    struct stat st;
    char pBuffer[MAX_RDWR_FILE_BUFF_SIZE];
    while((dir = readdir(pDir)) != NULL)
    {
        if(strcmp(dir->d_name,".") == 0
                || strcmp(dir->d_name,"..") == 0)//两个字符串比较  相等
        {
            continue;
        }    
        string sub_path = strQueueDir;
        sub_path=sub_path + "/" + dir->d_name;
        if(lstat(sub_path.c_str(),&st) == -1)//返回的是该符号链接本身的信息
        {
            continue;//获取失败
        }    
        if(S_ISDIR(st.st_mode))//判断该路径是否为目录  st_mode 文件访问权限
        {
            //检查数据文件是否存在
...
{
    string strIndexName=ConvertIndexToString(iMsgIndex);//将消息下标转换为字符串，不足10位的前面补0
    ...
    while((dir = readdir(pDir)) != NULL)
    {
        ...
        sub_path=sub_path + "/" + dir->d_name;
        if(lstat(sub_path.c_str(),&st) == -1)//文件返回的信息存在st内
        {
            continue;
        } 
        string strName=dir->d_name;   
        if(S_ISREG(st.st_mode))//判断文件是否为标准文件
        {
            ...
            strName=strName.substr(0,iPos);//拷贝index前的名称
...
{
    string strIndex=to_string(iIndex);//转换为字符串
    int count=10-strIndex.size();
    if(count>0)//在不足十位的前面补足0
    {
...
int LogicServer::FindPosInIndexFile(int iMsgIndex,const char *ipFile)
{
    ...
    struct stat buf;
    if(fstat(fd,&buf)==-1||buf.st_size<=0)//通过文件描述符得到文件的属性
    {
        return -1;
    }
    ...
    munmap(pStart,buf.st_size);//munmap()用来取消参数start 所指的映射内存起始地址，参数length 则是欲取消的内存大小
    return iPos;
}
...
int LogicServer::ProcessDataFile(const char *ipFile,int &oLastIndex,int iStartPos/*=0*/)
{
    ...
    while(iStartPos<buf.st_size)
    {
        ...
        //查找队列是否存在
        MessageQueue *pMsgQueue=NULL;
        if(m_mUnSubcribeQueues.find(strQueueName)!=m_mUnSubcribeQueues.end())//在根据名称保存的未订阅队列对象中找
        {
            pMsgQueue=m_mUnSubcribeQueues[strQueueName];
        }
        if(m_mSubcribeQueues.find(strQueueName)!=m_mSubcribeQueues.end())//在根据名称保存的订阅队列对象中找
        {
            pMsgQueue=m_mSubcribeQueues[strQueueName];
        }
        if(pMsgQueue==NULL)
        ...
        pMsgQueue->AddMessage(pMsg);//将消息保存在队列中
    }
...
int LogicServer::ProcessDurableMsgFile(const char *ipFile)
{
    ...
    {
        struct stat buf;
        if(fstat(consumeFd,&buf)==0&&buf.st_size>0)//读取文件信息成功
        {
...
{
    unordered_map<string,MessageQueue *>::iterator itr=m_mSubcribeQueues.begin();//根据名称保存的订阅队列对象
    while (itr!=m_mSubcribeQueues.end())
    {
        MessageQueue *pMsgQueue=itr->second;
        ++itr;
        //检查一下是否订阅
        if(!pMsgQueue->IsSubscribe())//队列订阅者为空
        {
            m_mSubcribeQueues.erase(itr++);//删除当前名称
        ...
        {
            SeverStoreMessage *pMsg=pMsgQueue->PopFrontMsg(true);//设置推送消息
            if(pMsg==NULL)
            {
                break;
            }
            ...
            pMsg->SetCmdId(CMD_SERVER_PUSH_MESSAGE);//服务端推送消息
            //若是需要确认，则消息放入待确认队列
            ...
            pMsg->GetMessagePack(pBuffer,&iLen);//推送消息
            ...
            if(pMsgQueue->GetConfirmLevel()==0)//不回应
            {
                //若消息是持久化的，则向持久化层发包
                if(pMsg->IsDurable())
                {
...
int LogicServer::OnPublishMessage(char *ipBuffer,int iLen,int iCliIndex,string &ostrMsgBody)
{
    ...
    //检查是否是未确认的重复消息
    if(confirmLevel>0&&m_producerMsgSeqs.find(strMsgSeq)!=m_producerMsgSeqs.end())//生产者发来的消息集合，格式为"生产者下标#消息编号"用于去重
    {
        ...
        //确认包入队
        m_pQueueToConnect->Enqueue(pAckPack,iLen);//和接入层通信的消息队列
    ...
    iLen-=offset;
    string strMsgBody;
    strMsgBody.assign(pTemp,iLen);//用一个字符串的前一段子串赋值
...
{
    unordered_map<int,int>::iterator itr=m_producerLastMsgSeq.begin();//批量确认时保存每一个客户发来的最新序号
...
int LogicServer::OnRecvMessage(char *ipBuffer,int iLen,int iCliIndex,string &ostrMsgBody)
{
    ...
    SeverStoreMessage*pMsg=pMsgQueue->PopFrontMsg(false);//设置不推送数据
    if(pMsg==NULL)
    {
    ...
    //消息放入队列
    while(m_pQueueToConnect->Enqueue(pBuffer,iLen)==ShmQueue::ERR_SHM_QUEUE_FULL)//和接入层通信的消息队列
    {
        //LOG_ERROR(0, 0, "push msg to consrv failed");
    }
...
int LogicServer::OnDeleteQueue(char *ipBuffer,int iLen,string &ostrMsgBody)
{
    ...
    while(itr!=m_mExchanges.end())
    {
        ...
        if(pExchange->GetBindingSize()==0)//根据binding key 绑定的queue的大小
        {
            itr++;
        }
        else
...
int LogicServer::OnCancelSubscribe(char *ipBuffer,int iLen,int iCliIndex,string &ostrMsgBody)
{
    ...
    if(pMsgQueue->IsAutoDel()&&!pMsgQueue->IsSubscribe())//队列的订阅者是否为空
    {
        pMsgQueue->ClearMessage();
        //遍历所有exchange，删除对应队列的绑定
        unordered_map<string,Exchange *>::iterator itr=m_mExchanges.begin();
...
int LogicServer::OnClientExit(int iClientIndex)
{
    ...
    unordered_set<string>::iterator itr1=m_producerMsgSeqs.begin();//生产者发来的消息集合，格式为"生产者下标#消息编号"（string类型）
    string strClientIndex=std::to_string(iClientIndex);
    while(itr1!=m_producerMsgSeqs.end())
    {
        string strMsgSeq=*itr1;
        if(strncmp(strClientIndex.c_str(),strMsgSeq.c_str(),strClientIndex.size())==0)//比较退出的是否是生产者
        {
    ...
    unordered_map<string,MessageQueue *>::iterator itr2=m_mSubcribeQueues.begin();//根据名称保存的订阅队列对象
    while(itr2!=m_mSubcribeQueues.end())
    {
        MessageQueue *pMsgQueue=itr2->second;
        int iOriNum=pMsgQueue->GetSubscribeSize();//返回队列的订阅者的大小
        pMsgQueue->CancelSubscribe(iClientIndex);//取消订阅
        int iNewNum=pMsgQueue->GetSubscribeSize();
        if(iOriNum>iNewNum)//取消之后队列订阅者大小减少
        {
            //说明该客户订阅了队列
    ...
    list<SeverStoreMessage *>::iterator itr3=m_NeedAckMessageList.begin();//保存的等待消费者确认的消息链表
    while(itr3!=m_NeedAckMessageList.end())
    {
        SeverStoreMessage *pMsg=*itr3;
        if(pMsg->GetClientIndex()==iClientIndex)//获取客户端下标
        {
            string strQueueName=pMsg->GetBelongQueueName();
            MessageQueue *pMsgQueue=NULL;
            if(m_mUnSubcribeQueues.find(strQueueName)!=m_mUnSubcribeQueues.end())//根据名称保存的未订阅队列对象
            {
                pMsgQueue=m_mUnSubcribeQueues[strQueueName];
            }
            if(m_mSubcribeQueues.find(strQueueName)!=m_mSubcribeQueues.end())//根据名称保存的订阅队列对象
            {
                pMsgQueue=m_mSubcribeQueues[strQueueName];
            }
            if(pMsgQueue)
            {
                pMsgQueue->AddMessageFront(pMsg);//把消息放回原来队列
            }
...
{
    ClientPackageHead header;//定义一个网络包头部
    //读取消息长度
    char *pTemp=ipBuffer;
    ...
    int ret=SUCCESS;
    switch(header.m_iCmdId)//按照消息类型创建
    {
