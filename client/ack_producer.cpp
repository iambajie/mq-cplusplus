...
string GetCurTime()
{
    ...
    string strRes=to_string(sNow.tv_sec);//秒
    strRes+="#";//#结尾
    strRes+=to_string(sNow.tv_usec);//微秒
    strRes+="#";
    return strRes;
}
...
int main(int argc,char *argv[])
{
    ...
    while(i<=100000)
    {
        ...
        int ret=pProducter->PuslishMessage("exchange1","mq.test2",strMsg);//交换机，路由键
        ...
        {
            if(pProducter->RecvServerAck()!=Client::SUCCESS)//接收数据包
            {
                producerLogger.WriteLog(mq_log_err,"recv ack failed!errMsg is %s",pProducter->GetErrMsg());
                return -1;
            }
            if(pProducter->ReSendMsg()!=Client::SUCCESS)//消息的确认与重发
            {
